...
function KeyTable ( oInit )
{
	...
	function _fnEventAddTemplate( sKey )
	{
		 ...
		 *         or
		 *           1. int:x - x coord. of target cell
		 *           2. int:y - y coord. of target cell
		...
		return function ( x, y, z ) {
			if ( typeof x == "number" && typeof y == "number" &&
			     typeof z == "function" )
			{
				_fnEventAdd( sKey, _fnCellFromCoords(x,y), z );
			}
			else if ( typeof x == "object" && typeof y == "function" )
			{
				_fnEventAdd( sKey, x, y );
			}
		};
	}
	...
	function _fnEventRemoveTemplate( sKey )
	{
		...
		return function ( x, y, z ) {
			if ( typeof arguments[0] == "number" && typeof arguments[1] == "number" )
			{
				...
				{
					_fnEventRemove( sKey, _fnCellFromCoords(x,y), z );
				}
				else
				{
					_fnEventRemove( sKey, _fnCellFromCoords(x,y) );
				}
			}
	 ...
	 *           node:nTarget - cell to add event too
	 *           function:fn - callback function for when triggered
	 */
	function _fnEventAdd( sType, nTarget, fn )
	{
	 ...
	 */
	function _fnEventRemove( sType, nTarget, fn )
	{
		var iCorrector = 0;
		...
		for ( var i=0, iLen=_oaoEvents[sType].length ; i<iLen-iCorrector ; i++ )
		{
			...
			{
				if ( _oaoEvents[sType][i-iCorrector].nCell == nTarget &&
			...
			{
				if ( _oaoEvents[sType][i].nCell == nTarget )
				{
	 ...
	 */
	function _fnEventFire ( sType, nTarget )
	{
		var iFired = 0;
	...
	{
	    /** by manuel **/
	    /* if the newTarget has no editable class, we'll return the oldFocused cell. */
	    if(!$(nTarget).hasClass("editable")) {
            return;
	    }
		...
		if ( _oDatatable )
		{
			...
			{
				$(oSettings.nNext).click();
			}
			
			while ( iRow < oSettings._iDisplayStart )
			{
				$(oSettings.nPrevious).click();
			}
		}
	...
	function _fnKey ( e )
	{
	    /* by manuel: because it's possible the cell has been moved, we need to reset the coords */
	    var aNewPos = _fnCoordsFromCell( _nOldFocus );
		_iOldX = aNewPos[0];
		_iOldY = aNewPos[1];
		...
		{
			case 13: /* return */
				_fnEventFire( "action", _nOldFocus );
				return true;
				
			case 27: /* esc */
				if ( !_fnEventFire( "esc", _nOldFocus ) )
				{
	...
	function _fnInit( oInit, that )
	{
		...
		if ( _oDatatable )
		{
			$('td', _oDatatable.fnGetNodes()).click( _fnClick );
		}
		else
		{
			$('td', _nBody).click( _fnClick );
		}
